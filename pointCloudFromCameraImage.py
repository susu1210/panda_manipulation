...
if __name__ == "__main__":
    p.connect(p.GUI)
    p.resetDebugVisualizerCamera(cameraDistance=0.5, cameraYaw=90, cameraPitch=-14,
                                 cameraTargetPosition=[0.60, 0.0, 0.45])
    p.setAdditionalSearchPath(pybullet_data.getDataPath())
    plane = p.loadURDF("plane100.urdf")
    ObjectName= 'YcbCrackerBox'
    cube = p.loadURDF(os.path.join(ycb_objects.getDataPath(), ObjectName, "model1.urdf"), [0, 0, 0.2])
    width, height, viewMat, projMat, cameraUp, camForward, horizon, vertical, _, _, dist, camTarget = p.getDebugVisualizerCamera()
    camPos = [
